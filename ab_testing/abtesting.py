...
class ABTest:
    def __init__(
        ...
        if group_column != None:
            ...
            if df_A[self.group_column].nunique() != 2:
                raise ValueError(
                    "Group column should contain two distinct values indicating A&B set"
                )
        ...
        # Check either DF B is given or group_column else raise the error.
            ...
            raise ValueError(
                "Either provide the second dataframe(df_B) or give column name for group indication of A&B"
            )
    ...
    def conversion_rate(self) -> pandas.DataFrame:
        ...
        conv_rt_report = pandas.DataFrame(
            data={
                "Conversion Rate": [conv_rt_a, conv_rt_b],
    ...
    def significance_test(self, threshold: float = 0.05) -> str:
        ...
        successes_of_groups = [
            self.a_sample[self.response_column].sum(),
            self.b_sample[self.response_column].sum(),
        ]
        ...
        (lower_con, lower_treat), (upper_con, upper_treat) = proportion_confint(
            successes_of_groups, nobs=[no_of_a, no_of_b], alpha=threshold
        )
        ...
        results = (
            f"z statistic: {z_stat:.2f}\tp-value: {pval:.3f}"
            f"\nConfidence Interval 95% for {self.a_label} group: {lower_con:.2%} to {upper_con:.2%}"
            f"\nConfidence Interval 95% for {self.b_label} group: {lower_treat:.2%} to {upper_treat:.2%}"
        ) + conclusion
